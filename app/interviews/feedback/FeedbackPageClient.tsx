...
const HERO_HEADLINE = "Precise feedback sharpens every release.";
const HERO_SUBCOPY =
  "Give focused product signal so we can triage requests with less back-and-forth. Structured answers hit the feedback table instantly and notify the team.";
...
export function FeedbackPageClient({
  ...
  const scrollToForm = () => {
    document.getElementById("feedback-form-root")?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  if (reduceMotion) {
    return (
      <InterviewsPageWidth className="space-y-8">
        ...
        <div className="relative overflow-hidden rounded-[var(--radius-lg)] border border-border/60 bg-gradient-to-br from-primary-soft via-background to-card shadow-[var(--shadow-1)]">
          ...
          <div className="relative px-6 py-8 sm:px-8 sm:py-10">
            ...
            <h2 className="krowe-display-m mt-4 max-w-3xl text-pretty text-foreground">
              Precise feedback sharpens every <span className="text-primary">release.</span>
            </h2>
        ...
        <div id="feedback-form-root">{children}</div>
      </InterviewsPageWidth>
    );
  }
  ...
  return (
    <InterviewsPageWidth className="space-y-8">
      ...
      >
        <div className="flex flex-col gap-3 border-b border-border/60 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Console</span>
            <Link
              href="/interviews/usage?range=24h"
      ...
      <motion.div
        initial={{ opacity: 0, y: 12 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: queueTitleDelay, duration: 0.26, ease: KROWE_EASE }}
      >
        <FeedbackInfoStrip />
      </motion.div>

      <motion.div
        id="feedback-form-root"
        initial={{ opacity: 0, y: 16 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: queueTitleDelay + 0.06, duration: 0.28, ease: KROWE_EASE }}
      >
        {children}
      </motion.div>
    </InterviewsPageWidth>
...
function FeedbackInfoStrip() {
  return (
    <section className="overflow-hidden rounded-[var(--radius-lg)] border border-border/80 bg-card px-5 py-4 shadow-[var(--shadow-1)] sm:px-6 sm:py-5">
      <div className="flex flex-wrap items-start gap-3">
        <MessageSquareIcon className="mt-0.5 h-5 w-5 shrink-0 text-primary" aria-hidden />
        <div>
          <p className="text-xs leading-relaxed text-muted-foreground">
            Your response powers product decisions. Submissions are stored securely and surfaced to the team through our
            usual review channels.
          </p>
          <div className="mt-3 flex flex-wrap gap-2 text-[11px]">
            <span className="rounded-full border border-primary/25 bg-primary-soft px-2.5 py-1 font-semibold text-primary">
              Stored in product_feedback
            </span>
            <span className="rounded-full border border-border/80 bg-surface-subtle px-2.5 py-1 font-semibold text-muted-foreground">
