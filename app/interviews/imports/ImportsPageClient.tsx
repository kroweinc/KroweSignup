...
export type ImportsPageTiming = {
  queueTitleDelay: number;
  enableEntrance: boolean;
  /** Entrance for Granola hub card (after overview block). */
  integrationHubDelay: number;
};
...
type Props = {
  ...
  statCards: ImportsStatCard[];
  readinessPct: number;
  queueCaption: string;
  children: (timing: ImportsPageTiming) => ReactNode;
};

const HERO_HEADLINE = "Route transcript signal into project decisions.";
const HERO_SUBCOPY =
  "Sync notes from Granola, triage in the inbox studio, and route each transcript into a workspace. Summaries appear here as assignable notes—skim fast, then assign.";
...
export function ImportsPageClient({
  ...
  const integrationHubDelay = overviewBlockDelay + 0.06;

  const timingReduced: ImportsPageTiming = {
    queueTitleDelay: 0,
    enableEntrance: false,
    integrationHubDelay: 0,
  };
  const timingFull: ImportsPageTiming = { queueTitleDelay, enableEntrance: true, integrationHubDelay };

  const scrollToInbox = () => {
    document.getElementById("imports-inbox")?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  if (reduceMotion) {
    return (
      <InterviewsPageWidth className="space-y-8">
        ...
        <div className="relative overflow-hidden rounded-[var(--radius-lg)] border border-border/60 bg-gradient-to-br from-primary-soft via-background to-card shadow-[var(--shadow-1)]">
          ...
          <div className="relative px-6 py-8 sm:px-8 sm:py-10">
            ...
            <h2 className="krowe-display-m mt-4 max-w-3xl text-pretty text-foreground">
              Route transcript signal into{" "}
              <span className="text-primary">project decisions.</span>
            </h2>
        ...
        <ImportsOverviewStatic metrics={metrics} readinessPct={readinessPct} queueCaption={queueCaption} />

        {children(timingReduced)}
      </InterviewsPageWidth>
    );
  }
...
function ImportsOverviewStatic({
  metrics,
  readinessPct,
  queueCaption,
}: {
  metrics: ImportsStatCard[];
  readinessPct: number;
  queueCaption: string;
}) {
  return (
